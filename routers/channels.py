...
from wa_evolution import EvolutionClient
from db import get_session, Session, select, WAConfig, Brand, WAChatMeta, ChannelAccount
...
def _get_instance_name(session: Session, brand_id: int) -> str:
    """
    Lee/guarda el nombre de instancia Evolution para la brand.
    Por defecto usa 'brand_{id}'. Si existe ChannelAccount(channel='wa'), lo respeta.
    """
    ca = session.exec(
        select(ChannelAccount).where(
            ChannelAccount.brand_id == brand_id,
            ChannelAccount.channel == "wa"
        )
    ).first()
    if ca and ca.external_id:
        return ca.external_id
    # default
    return f"brand_{brand_id}"

def _save_instance_name(session: Session, brand_id: int, instance: str):
    ca = session.exec(
        select(ChannelAccount).where(
            ChannelAccount.brand_id == brand_id,
            ChannelAccount.channel == "wa"
    ...
    if not ca:
        ca = ChannelAccount(brand_id=brand_id, channel="wa", external_id=instance)
    else:
        ca.external_id = instance
    session.add(ca); session.commit(); session.refresh(ca)
...
def wa_config(brand_id: int = Query(...), session: Session = Depends(get_session)):
    ...
    cfg = session.exec(select(WAConfig).where(WAConfig.brand_id == brand_id)).first()
    has_pw = bool(getattr(cfg, "super_password_hash", None))
    instance_name = _get_instance_name(session, brand_id)
    return {
        "brand": {"id": brand.id if brand else brand_id, "name": (brand.name if brand else f"brand_{brand_id}")},
        "config": cfg,
        "datasources": [],
        ...
        "webhook_example": f"{PUBLIC_BASE_URL}/api/wa/webhook?token={EVOLUTION_WEBHOOK_TOKEN}&instance={instance_name}",
        "server_caps": {
            "can_list_chats": False,   # tu server actual devuelve 404 en listar chats
            "has_webhook_set": False,  # se intentará setear abajo; si 404 seguirá False
        }
    }

# ---------------- Conexión / QR ----------------
@router.post("/start")
def wa_start(brand_id: int = Query(...), session: Session = Depends(get_session)):
    if not EVOLUTION_BASE_URL:
        raise HTTPException(500, "EVOLUTION_BASE_URL no configurado")
    if not PUBLIC_BASE_URL:
        raise HTTPException(500, "PUBLIC_BASE_URL no configurado")
    ...
    evo = EvolutionClient()
    instance = _get_instance_name(session, brand_id)
    webhook_url = f"{PUBLIC_BASE_URL}/api/wa/webhook?token={EVOLUTION_WEBHOOK_TOKEN}&instance={instance}"
    ...
    try:
        ...
        log.warning("create_instance fallo: %s", e)

    # 2) intenta setear webhook (en muchos servers devuelve 404/400)
    sc, wjs = evo.set_webhook(instance, webhook_url)
    if sc >= 400:
        log.warning("set_webhook fallo (%s). Si tu server no soporta setear webhook, "
                    "usa /api/wa/instance/rotate para crear una nueva instancia con webhook al crear. Detalle: %s", sc, wjs)
    ...
    return {"ok": True, "webhook": webhook_url, "instance": instance, "set_webhook_status": sc, "set_webhook_body": wjs}

@router.get("/qr")
def wa_qr(brand_id: int = Query(...), session: Session = Depends(get_session)):
    if not EVOLUTION_BASE_URL:
        raise HTTPException(500, "EVOLUTION_BASE_URL no configurado")
    evo = EvolutionClient()
    instance = _get_instance_name(session, brand_id)
...
# --------- Instance helpers (debug/rotación) ----------
@router.get("/instance/status")
def wa_instance_status(brand_id: int = Query(...), session: Session = Depends(get_session)):
    evo = EvolutionClient()
    instance = _get_instance_name(session, brand_id)
    st = evo.connection_state(instance)
    return {"instance": instance, "state": st}

@router.post("/instance/rotate")
def wa_instance_rotate(brand_id: int = Query(...), session: Session = Depends(get_session)):
    """
    Workaround para servers Evolution que NO permiten setWebhook después:
    crea una nueva instancia con el webhook puesto en el create y la guarda en ChannelAccount.
    """
    if not PUBLIC_BASE_URL:
        raise HTTPException(500, "PUBLIC_BASE_URL no configurado")
    evo = EvolutionClient()
    new_instance = f"brand_{brand_id}_{int(time.time())}"
    webhook_url = f"{PUBLIC_BASE_URL}/api/wa/webhook?token={EVOLUTION_WEBHOOK_TOKEN}&instance={new_instance}"
    evo.create_instance(new_instance, webhook_url=webhook_url)
    # guardar en DB como instancia activa
    _save_instance_name(session, brand_id, new_instance)
    # conectar
    try:
        evo.connect_instance(new_instance)
    except Exception as e:
        log.warning("connect_instance(%s) error: %s", new_instance, e)
    return {"ok": True, "instance": new_instance, "webhook": webhook_url}
...
def wa_chats(brand_id: int = Query(...), limit: int = Query(200, ge=1, le=2000), session: Session = Depends(get_session)):
    ...
    status, js = evo.list_chats(instance, limit=limit)
    # muchos servers devuelven 404 -> vaciamos sin romper y adjuntamos raw
    chats = _extract_chats_payload(js if status < 400 else {})
    return {"ok": status < 400, "status": status, "raw": js, "chats": chats}
...
# ---------------- Test envío ----------------
@router.post("/test")
def wa_test(body: Dict[str, Any], session: Session = Depends(get_session)):
    brand_id = int(body.get("brand_id") or 0)
    to = (body.get("to") or "").strip()
    text = body.get("text") or "Hola desde API"
    if not brand_id or not to:
    ...
    resp = evo.send_text(instance, to, text)  # {"http_status": int, "body": dict}
    http_status = int(resp.get("http_status") or 500)
    if http_status >= 400:
        raise HTTPException(http_status, str(resp.get("body")))
    return {"ok": True, "result": resp.get("body")}
...
def wa_board(
    ...
    # Si el server no lista chats (404), podés igualmente “sembrar” tarjetas
    # creando metas manuales (via /chat/meta) y aparecerán acá si hacés enriched desde metas.
    # (No lo forzamos por defecto para no mezclar ruido)
